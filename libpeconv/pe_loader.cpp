...
BYTE* peconv::load_pe_module_dll(BYTE* dllRawData, size_t r_size, OUT size_t &v_size, bool executable, bool relocate)
{
...
BYTE* peconv::load_pe_module(const char *filename, OUT size_t &v_size, bool executable, bool relocate)
{
    ...
    BYTE* mappedDLL = load_pe_module_dll(dllRawData, r_size, v_size, executable, relocate);
    free_pe_buffer(dllRawData);
    return mappedDLL;
}

BYTE* peconv::load_pe_executable_dll(BYTE* dllRawData, size_t r_size, OUT size_t &v_size, t_function_resolver* import_resolver)
{
    BYTE* loaded_pe = load_pe_module_dll(dllRawData, r_size, v_size, true, true);
    if (!loaded_pe) {
